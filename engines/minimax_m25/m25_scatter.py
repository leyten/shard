...
"""
import re, sys, json, time, shlex, secrets, subprocess, argparse
...
def fresh_count(host, port, nonce_file, nonce, log, pattern):
    """count PATTERN matches in LOG only when the box's nonce file matches THIS launch (M4): a
    stale log left by a previous run — e.g. after a flaked ssh that never ran the launch — can
...
def sidecar_cmd(announce, inbound, forward, nonce=""):
    ...
    return (f"pkill -9 -x sidecar 2>/dev/null; fuser -k {LIBP2P}/tcp {FWD}/tcp 29612/tcp 2>/dev/null; sleep 2; rm -f /root/sidecar.log; {nn}"
            f"setsid bash -c {shlex.quote(inner)} </dev/null >/dev/null 2>&1 &")


def launch_sidecar(host, port, announce, inbound, forward):
    nonce = secrets.token_hex(8)
    cmd = sidecar_cmd(announce, inbound, forward, nonce=nonce)
    for attempt in range(5):
        sh(host, port, cmd, 30)
        for _ in range(4):
            time.sleep(3)
            # require BOTH the tunnel up AND no forward-bind failure (the bug that broke the ring)
            last = fresh_count(host, port, "/root/sidecar.nonce", nonce, "/root/sidecar.log", "tunnel up|listening")
            badn = fresh_count(host, port, "/root/sidecar.nonce", nonce, "/root/sidecar.log", "address already in use")
...
def launch_stage(host, port, lo, hi, is_tail):
    ...
    cmd = (f"nvidia-smi --query-compute-apps=pid --format=csv,noheader | xargs -r kill -9 2>/dev/null; "
           f"fuser -k {ENG_IN}/tcp 2>/dev/null; sleep 4; rm -f /root/stage.log; echo {nonce} > /root/stage.nonce; cd /root && "
...
def warm(host, port, label, nonce, tries=70):
...
def main():
    ...
    for k in range(n - 1, -1, -1):
        nd = nodes[k]
        nonces[k] = launch_stage(nd["host"], nd["port"], nd["lo"], nd["hi"], is_tail=(k == n - 1))
    for k in range(n - 1, -1, -1):
        nd = nodes[k]
        ok = warm(nd["host"], nd["port"], f"stage{k} {nd['region']}", nonces[k])
    ...
    cmd = (f"set -o pipefail; cd /root && CUDA_VISIBLE_DEVICES=0 M25_DIR=/root/m25 /root/venv/bin/python /root/m25_ring.py coord "
    ...
    if r.returncode != 0:
        print(f"[scatter] coordinator FAILED (rc={r.returncode})", flush=True)
        sys.exit(1)
